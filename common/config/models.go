...
type Heros struct {
	...
	RAsMap map[uint8][]uint32 `excel:"rAsMap"` //回合-能力list
	Price  map[int]uint64     `excel:"price"`  // 自动解析逗号分隔: tag1,tag2,tag3
...
type Item struct {
	...
	Price    map[int]uint64 `excel:"price"`    // 价值
...
type ReceiveAward struct {
	Id         int            `excel:"id"`         // 道具唯一id
	RewardMap  map[int]uint64 `excel:"rewardMap"`  // 价值
