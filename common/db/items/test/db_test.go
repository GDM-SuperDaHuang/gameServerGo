...
func TestRun2(t *testing.T) {
	...
	for i := 0; i < goroutineCount; i++ {
		...
		go func(index int) {
			...
			if err := saveWithRetry(ctx, 123456, 10, 2); err != nil {
				t.Errorf("协程 %d 执行失败: %v", index, err)
			}
			if err := saveWithRetry(ctx, 123456, 10, -1); err != nil {
				t.Errorf("协程 %d 执行失败: %v", index, err)
			}
			if err := saveWithRetry(ctx, 123456, 20, 1); err != nil {
				t.Errorf("协程 %d 执行失败: %v", index, err)
			}
		}(i)
	}
...
func saveWithRetry(ctx context.Context, roleID uint64, itemID int, count int64) error {
	const maxRetries = 3
	_, err := items.AddItem(roleID, itemID, count)
	if err != nil {
		return err
	}
	...
	//			continue
	//		}
	//		return fmt.Errorf("添加物品失败 (尝试%d次): %w", maxRetries, err)
	//	}
	//	return nil
	//}
	return nil
