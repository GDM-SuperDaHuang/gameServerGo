...
// 封包
func (l *deEnCode) Pack(message *common.Message, cryptoHandler Crypto) (Callback, []byte, error) {
...
// Unpack 解包
func (l *deEnCode) Unpack(reader Reader) ([]*common.Message, error) {
	messages := make([]*common.Message, 0, 4)
...
// 封包
func (l *deEnCode) packBody(message *common.Message, cryptoHandler Crypto) ([]byte, uint16, error) {
	// 负载
	body := message.Body
	if len(body) == 0 {
		return nil, 0, nil
...
func (l *deEnCode) unpackOneMessage(reader Reader) (*common.Message, error) {
	// 读取完整消息
	...
	return common.NewMessage(head.Flag, head.SN, head.Code, head.Protocol, payload), nil
}

// 解析头部
func (l *deEnCode) parseMessageHead(allBytes []byte) (*common.MessageHead, int, error) {
	index := lenFieldLength
	head := &common.MessageHead{
...
//// NewMessage 创建消息
//func NewMessage(flag uint16, sn uint32, code uint16, protocol uint16, payload []byte) *common.Message {
//	m := messagePool.Get()
...
//}
//
//func NewMessageResp(resp *common.Resp, message *common.Message) *common.Message {
//	m := messagePool.Get()
//	m.Head = &common.MessageHead{
...
//
//// 释放消息
//func FreeMessage(m *common.Message) {
//	messagePool.Put(m)
//}
//
//// messagePool 消息池
//var messagePool = common.NewPool(func() *common.Message {
